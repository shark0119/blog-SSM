...
var oViewMore = document.getElementsByClassName('view-more');
var oChildComments = document.getElementsByClassName('child-comments');
...
// 初始化检测oChildComments有无子节点 无则去掉边框
for (var i=0;i<oChildComments.length;i++){
	if (oChildComments[i].childElementCount < 1){
		oChildComments[i].style.borderColor = 'transparent';
	}
}
...
//点击查看更多 显示子评论
for (var s= 0,len=oViewMore.length;s<len;s++){
	oViewMore[s].sta = false;
	oViewMore[s].onclick = function(){
		this.sta = !this.sta;
		if (this.sta){
